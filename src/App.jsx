...
import { Card, ToastContainer, ErrorBoundary } from "./components/UIComponents";
...
export default function App() {
  ...
  const [isSessionChecked, setIsSessionChecked] = useState(false); // New state
  ...
  const handleLogout = () => {
    ...
    setUser(null);
    // Optional: Reset activeAppId to default if you want, 
    // but keeping it might be safer for immediate re-login. 
    // For now, let's reset to default to be clean.
  ...
  useEffect(() => {
    ...
    // 3. We have a valid reason to fetch (Admin logged in OR Scanning mode)
    if (!user || !isSessionChecked) return;
    if (!adminLoggedIn && !activeTeamId) return;

    setIsDataLoaded(false);

    // Use activeAppId which is now correctly set
  ...
  }, [user, adminLoggedIn, activeAppId, view, activeTeamId, isSessionChecked]);
